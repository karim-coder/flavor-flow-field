...
export const SingleKPattern = ({
  strokeColor = "#645741",
  fillColor = "rgba(211, 162, 77, 0.8)",
  strokeWidth = "10",
  backgroundColor = "bg-[#F5DEB3]",
  patternSize = "100px",
  rotation = 0, // New prop for rotation in degrees
  children = null,
  className = "",
  ...props
}: {
  ...
  const kWidth = 1500;
  const kHeight = 1400;
  const padding = 50; // Adding some padding around the K

  // Calculate total dimensions with padding
  const totalWidth = kWidth + padding * 2;
  const totalHeight = kHeight + padding * 2;

  // Create SVG with just the K3 (horizontally flipped K)
  const encodedSvg = encodeURIComponent(`
    <svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${totalHeight}" viewBox="0 0 ${totalWidth} ${totalHeight}">
      <rect x="0" y="0" width="${totalWidth}" height="${totalHeight}" fill="none" />
      
      <!-- Single K with horizontal flip and rotation -->
      <g transform="translate(${totalWidth / 2}, ${
    totalHeight / 2
  }) rotate(${rotation}) scale(-1,1) translate(${-kWidth / 2},${-kHeight / 2})">
        <path d="${kSvgPath}" fill="${fillColor}" stroke="${strokeColor}" stroke-width="${strokeWidth}"/>
      </g>
    </svg>
  `);
